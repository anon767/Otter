...
extern void __otter_copy_unescaped_string_bounds_checking(char const *base, char const *cur);
#include <config.h>
...
copy_unescaped_string (const char *string)
{
  ...
  while (__otter_copy_unescaped_string_bounds_checking(original_string, string), *string)
    if (*string == '\\')
      {
	string++;
	switch (*string)
