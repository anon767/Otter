...
for (key,val) in sorted(stat_coverage.items(),key=lambda (k,v):-len(v|stat_coverage_inherited[k])):
	if val==set(): continue
	configstr = ""
	for k in sorted(key): configstr+= k+","
	print "%d\t%d\t%s" % (len(val|stat_coverage_inherited[key]),len(key),configstr[:-1])
