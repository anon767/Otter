...
type covType = Line | Block | Condition | Edge | Path
let string_of_covType = function
	| Line -> "line"
	| Block -> "block"
	| Condition -> "condition"
	| Edge -> "edge"
...
let rec printTree_aux indentLevel : varDepTree -> unit = function
	| Node ((bytes,cov),children) ->
			Format.printf "%s%s (%d children)\n" (String.make indentLevel '\t') (To_string.humanReadableBytes !bytesToVars bytes) (List.length children);
			List.iter (printTree_aux (succ indentLevel)) children

let printTree = printTree_aux 0
*)
...
(*	let assignments = List.sort Pervasives.compare assignments in*)
	...
	if not (CovSet.is_empty result) then (
		...
		List.iter
			...
			assignments;
		Format.printf "these %d %ss are hit\n" (CovSet.cardinal result) (string_of_covType CovSet.covType);
...
let calculateDeps treeOfPcs toIgnoreFile configsToTry =
	...
	Format.printf "\n%d %ss always executed:\n" (CovSet.cardinal alwaysExecuted) (string_of_covType CovSet.covType);
	...
	(try
		 while true do
			 Format.printf "\n%d %ss left\n" (CovSet.cardinal !remainingCovUnits) (string_of_covType CovSet.covType);
...
let coverageType = ref Line
let setCoverageType s =
	match String.lowercase s with
			"line" -> coverageType := Line
		| "edge" -> coverageType := Edge
		| "block" -> coverageType := Block
		| "cond" -> coverageType := Condition
		| "path" -> coverageType := Path
		| _ -> failwith "Unknown coverage type"

let speclist = [
	("--type",
	 Arg.String setCoverageType,
	 "<coverageType> Which type of coverage: line, block, edge, cond, or path. Default is line.");
...
(* Having such special treatment feels quite inelegant, but we certainly don't
	 want to naively do the full calculation. *)
let addCoverageFromFileForPaths filename =
	let inChan = open_in_bin filename in
	(* Read in the coverage data structure *)
	let jobResults = (Marshal.from_channel inChan : job_result list) in
	close_in inChan;
...
let main () =
	...
	match !coverageType with
		...
		| Path ->
				List.iter addCoverageFromFileForPaths !coverageFiles
;;
