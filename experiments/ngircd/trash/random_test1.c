...
char	CLIENT_WHATEVER[MAX_CLIENT][1024]; //	"WHATEVER #group\r\n"
...
void sym_test_1 (void* param) {
	...
	{	
		...
		{
			char c = __SYMBOLIC();
			if(k<10)__ASSUME(OR(c==' ',AND(c>='A',c<='Z')));
			else __ASSUME(c>=32,c<=127);
			CLIENT_WHATEVER[j][k] = c;
		}
		__ASSUME(
				NOT(AND(
				CLIENT_WHATEVER[j][0]=='T',
				CLIENT_WHATEVER[j][1]=='I',
				CLIENT_WHATEVER[j][2]=='M',
				CLIENT_WHATEVER[j][3]=='E'
				))
				);
		CLIENT_WHATEVER[j][k++] = '\r';
		CLIENT_WHATEVER[j][k++] = '\n';
		CLIENT_WHATEVER[j][k++] = '\0';
	}
		...
		if((--this_poll_call_count)==0){
			...
			for(int j=0;j<NUM_WHATEVER;j++){
				if((--this_poll_call_count)==0){
					...
					// the clinet wanna send WHATEVER
					fds[client_fd].revents = POLLIN;
				 	return 1;
				}else if((--this_poll_call_count)==0){
				 	// now the server should respond
		...
		if((--this_read_call_count)==0){
			...
			for(int j=0;j<NUM_WHATEVER;j++){
				if((--this_read_call_count)==0){
					// SEND WHATEVER
					__ASSERT(fildes==client_fd);
					strcpy(buf,CLIENT_WHATEVER[i]);
					return strlen(CLIENT_WHATEVER[i]);
				//}else if((--this_read_call_count)==0){
				//	// SEND PART
				//	__ASSERT(fildes==client_fd);
