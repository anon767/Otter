...
void sym_test_1 (void* param) {
	...
	for(int i=0;i<3;i++)
	{
		char symbol = __SYMBOLIC(1); // symbolic
		__ASSUME(symbol>='a',symbol<='z');
		GROUP_NAME[1+i] = symbol;
	}
	char SYM_JOIN[20];
	sprintf(SYM_JOIN,"JOIN %s\r\n",GROUP_NAME);
	// join
	for(int j=0;j<NUM;j++)
	{
		CLIENT_JOIN[j] = SYM_JOIN;
	}
	// privmsg
	//char SYM_PRIVMSG0[100];
	//sprintf(SYM_PRIVMSG0,"PRIVMSG %s :Hello\r\n",GROUP_NAME);
	//CLIENT_PRIVMSG[0] = SYM_PRIVMSG0;
	//char SYM_PRIVMSG1[100];
	//sprintf(SYM_PRIVMSG1,"PRIVMSG %s :World\r\n",GROUP_NAME);
	//CLIENT_PRIVMSG[1] = SYM_PRIVMSG1;
	// part
	char SYM_PART[100];
	sprintf(SYM_PART,"PART %s\r\n",GROUP_NAME);
	CLIENT_PART[0] = SYM_PART;
	char SYM_QUIT[100];
	sprintf(SYM_QUIT,"QUIT\r\n",GROUP_NAME);
	CLIENT_QUIT[0] = SYM_QUIT;
...
int poll(struct pollfd fds[], nfds_t nfds, int timeout){
	...
	// 	return 1;
		...
		// now the server should respond - JOIN of 9
		assert(fds[5].events == POLLOUT);
		fds[5].revents = POLLOUT;
		return 1;
	// }else if((--this_call_count)==0){
	...
	// 	return 1;
		...
		// the client wanna send PART
		// (ASSUME server doesn't reply)
		fds[9].revents = POLLIN;
		return 1;
	}else if((--this_call_count)==0){
		// now the server should respond - PART of 9
		assert(fds[5].events == POLLOUT);
		fds[5].revents = POLLOUT;
		return 1;
	}else if((--this_call_count)==0){
...
ssize_t read(int fildes, void *buf, size_t nbyte){
	...
	//	// SEND NICK/USER
	//	assert(fildes==13);
	//	strcpy(buf,CLIENT_NICK[2]);
	//	return strlen(CLIENT_NICK[2]);
	//}else if((--this_call_count)==0){
	//	// SEND USER/NICK
	//	assert(fildes==13);
	//	strcpy(buf,CLIENT_USER[2]);
	//	return strlen(CLIENT_USER[2]);
	}else if((--this_call_count)==0){
		// SEND JOIN
		assert(fildes==5);
		...
		// SEND JOIN
		assert(fildes==9);
		strcpy(buf,CLIENT_JOIN[1]);
		return strlen(CLIENT_JOIN[1]);
	//}else if((--this_call_count)==0){
	//	// SEND JOIN
	//	assert(fildes==13);
	//	strcpy(buf,CLIENT_JOIN[2]);
	//	return strlen(CLIENT_JOIN[2]);
	//}else if((--this_call_count)==0){
	//	// SEND PRIVMSG
	//	assert(fildes==5);
	//	strcpy(buf,CLIENT_PRIVMSG[0]);
	//	return strlen(CLIENT_PRIVMSG[0]);
	//}else if((--this_call_count)==0){
	//	// SEND PRIVMSG
	//	assert(fildes==9);
	//	strcpy(buf,CLIENT_PRIVMSG[1]);
	//	return strlen(CLIENT_PRIVMSG[1]);
	}else if((--this_call_count)==0){
		// SEND PART
		assert(fildes==9);
		strcpy(buf,CLIENT_PART[0]);
		return strlen(CLIENT_PART[0]);
	}else if((--this_call_count)==0){
		// SEND QUIT
		assert(fildes==9);
		strcpy(buf,CLIENT_QUIT[0]);
		return strlen(CLIENT_QUIT[0]);
	//}else if((--this_call_count)==0){
	//	// SEND PRIVMSG
	//	assert(fildes==5);
	//	strcpy(buf,CLIENT_PRIVMSG[0]);
