...
int gethostname(char *name, size_t namelen){
	strcpy(name,"return@gethostname");
	return 0;
}
//#endif
...
//#if __HAVE_pipe__
int (*pipe_fildes0_callback)(int) = 0;
int pipe(int fildes[2]){
	fildes[0] = socket(0,0,0); 
	if(pipe_fildes0_callback!=0)
		pipe_fildes0_callback(fildes[0]);
	fildes[1] = socket(0,0,0); // never use coz we don't really fork a child
	return 0;
}
//#endif
