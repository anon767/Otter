...
module BidirectionalLabeled(V: COMPARABLE)(E: ORDERED_TYPE)(HM: HM with type key = V.t) = struct
  ...
  let mem_edge_e g (v1, l, v2) =
    try let ve = v2, l in S.exists (fun ve' -> VE.compare ve ve' == 0) (snd (HM.find v1 g))
    with Not_found -> false

  exception Found of edge
  let find_edge g v1 v2 =
    try S.iter (fun (v2', l) -> if V.equal v2 v2' then raise (Found (v1, l, v2'))) (snd (HM.find v1 g));
        raise Not_found
    with Found e -> e

  let unsafe_add_edge g v1 (v2, l) =
    let (in_set,out_set) = HM.find v1 g in
  ...
  let unsafe_remove_edge g v1 v2 =
    let (in_set,out_set) = HM.find v1 g in
    let g = HM.add v1 (in_set, (S.filter (fun (v2', _) -> not (V.equal v2 v2')) out_set)) g in
    let (in_set,out_set) = HM.find v2 g in
    HM.add v2 (S.filter (fun (v1', _) -> not (V.equal v1 v1')) in_set, out_set) g
  ...
  let remove_edge g v1 v2 =
    if not (HM.mem v2 g && HM.mem v1 g) then invalid_arg "[ocamlgraph] remove_edge";
  ...
  let iter_succ f g v = S.iter (fun (w, _) -> f w) (snd (HM.find_and_raise v g "[ocamlgraph] iter_succ"))
  let fold_succ f g v = S.fold (fun (w, _) -> f w) (snd (HM.find_and_raise v g "[ocamlgraph] fold_succ"))

  let iter_succ_e f g v = S.iter (fun (w, l) -> f (v, l, w)) (snd (HM.find_and_raise v g "[ocamlgraph] iter_succ_e"))
  let fold_succ_e f g v = S.fold (fun (w, l) -> f (v, l, w)) (snd (HM.find_and_raise v g "[ocamlgraph] fold_succ_e"))

  let succ g v = fold_succ (fun w l -> w :: l) g v []
  let succ_e g v = fold_succ_e (fun e l -> e :: l) g v []

  let map_vertex f = HM.map begin fun v (s1, s2) ->
    let s1 = S.fold (fun (v, l) s -> S.add (f v, l) s) s1 S.empty in
    let s2 = S.fold (fun (v, l) s -> S.add (f v, l) s) s2 S.empty in
    f v, (s1, s2)
  end
  ...
  module I = struct
    ...
    module PV = V
    module PE = E
    let iter_edges f = HM.iter (fun v (_, outset) -> S.iter (fun (w, _) -> f v w) outset)
    let fold_edges f = HM.fold (fun v (_, outset) -> S.fold (fun (w, _) -> f v w) outset)
    let iter_edges_e f = HM.iter (fun v (_, outset) -> S.iter (fun (w, l) -> f (v, l, w)) outset)
    let fold_edges_e f = HM.fold (fun v (_, outset) -> S.fold (fun (w, l) -> f (v, l, w)) outset)
  end
  include I

  let iter_pred f g v = S.iter (fun (w, _) -> f w) (fst (HM.find_and_raise v g "[ocamlgraph] iter_pred"))
  let fold_pred f g v = S.fold (fun (w, _) -> f w) (fst (HM.find_and_raise v g "[ocamlgraph] fold_pred"))

  let iter_pred_e f g v = S.iter (fun (w, l) -> f (w, l, v)) (fst (HM.find_and_raise v g "[ocamlgraph] iter_pred_e"))
  let fold_pred_e f g v = S.fold (fun (w, l) -> f (w, l, v)) (fst (HM.find_and_raise v g "[ocamlgraph] fold_pred_e"))
