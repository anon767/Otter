 ...
 * The original program actually runs fine with ratio > 1.
 * But a bug inserted in version 6 breaks the guard in the for-loop of
...
int OTTER_SYMBOLIC_FLOAT() {
    // OTTER doesn't support symbolic float.
    // Normally the program expects [0.0, 1.0]
    // but it has no checking
...
int ratio;
{
    ...
    // OTTER: ratio must be >= 1.0 so that n == count + 1
	n = (int) (count*ratio + 1);
...
void main(void)
{
    ...
    for (status = (command = OTTER_SYMBOLIC_INT());
	 ((status!=EOF) && status);
	 status = (command = OTTER_SYMBOLIC_INT()))
    {
	switch(command)
	{
	case FINISH:
