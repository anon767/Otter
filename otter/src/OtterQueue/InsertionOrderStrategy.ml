...
class ['self] t = object (_ : 'self)
    ...
    method add job =
        ...
        let queue = JobOrder.insert job#jid (rank - 1) () queue in
        {< queue = queue >}

    method remove job =
        try
            (* remove the job and shift the rank of all jobs more recent than that job *)
            let rank = fst (JobOrder.lookup job#jid queue) in
            let queue = JobOrder.delete job#jid queue in
    ...
    method weight job =
        try
            ...
            -1. /. float_of_int (fst (JobOrder.lookup job#jid queue))
        with JobOrder.Key ->
            raise Not_found
end
