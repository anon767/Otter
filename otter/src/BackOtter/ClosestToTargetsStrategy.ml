...
let get_distance_to_targets_impl =
    ...
    Memo.memo "ClosestToTargetsStrategy.get_distance_to_targets_impl" 
        (fun (job, target_fundecs, line_targets) ->
...
class ['self] t = object (self : 'self)
    ...
    method weight job =
        let distance = get_distance_to_targets job in 
