        ...
        match bytes with
            ...
            * *)
                ...
                | Bytes_Read(bytes,off,len) -> 
                        conditional__map (deref state) (expand_read_to_conditional state bytes len off)
...
(*
let rec prune_conditional state cond =
        match cond with
                | IfThenElse (guard, x, y) ->
                        let rec evalGuard guard =
                                match guard with
                                        | Guard_True -> true
                                        | Guard_Not g -> lnot (evalGuard g)
                                        | Guard_And (x, y) -> (evalGuard x) land (evalGuard y)
                                        | Guard_Or (x, y) -> (evalGuard x) lor (evalGuard y)
                                        | Guard_Symbolic _-> true
                                        | Guard_Bytes b ->
                                                match b with
                                                        | Bytes_Op (op, args) ->
                                                        | _ -> failwith "fail guard expression"
                        in
                        let truth = (Stp.eval state.path_condition guard) in
                        if truth == True then
                                prune_conditional state x
                        else if truth == False then
                                prune_conditional state y
                        else
                                IfThenElse(guard, (prune_conditional x), (prune_conditional y))
                | Unconditional _ -> cond
;;
*)
