	...
	| Cil.Call(retopt, Cil.Lval(Cil.Var(varinfo), Cil.NoOffset), _, _)::_ when varinfo.Cil.vname = "fork" ->
		...
		let job, child_job = match retopt with
			...
			| Some cil_lval ->
				...
				(job, child_job)
		in
		(Driver.step_job job, put_job child_job multijob)
...
let main_loop job =
	...
	(* main loop that runs a queue of multijobs one step at a time *)
	let rec main_loop completed = function
		| multijob::multijob_queue ->
			(* get a job and run it *)
			begin match get_job multijob with
				| None ->
					main_loop completed multijob_queue
				| Some (job, multijob) ->
					let job_states, multijob = step_multijob job multijob in

					(* process the results *)
					let rec process_job_states completed multijob_queue = function
						| Types.Active job ->
						...
						| Types.Big_Fork states ->
							...
							List.fold_left begin fun (completed, multijob_queue) state ->
								process_job_states completed multijob_queue state
	...
	(* initial multijob which contains just the given job *)
	let multijob = {
		processes = [];
		shared = {
			path_condition = [];
