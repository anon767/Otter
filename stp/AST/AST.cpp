...
//    } else 
  ...
  void BeevMgr::Push(void) {
    ASTVecPtr v(new ASTVec());
    _asserts.push_back(v);
  }
  
  void BeevMgr::Pop(void) {
    if(!_asserts.empty()) {
       ASTVecPtr c(_asserts.back());
  ...
  const ASTVec BeevMgr::GetAsserts(void) {
    vector<ASTVecPtr>::iterator it = _asserts.begin();
    vector<ASTVecPtr>::iterator itend = _asserts.end();
