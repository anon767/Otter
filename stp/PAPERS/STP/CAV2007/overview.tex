...
%% \begin{description}
%% \item[\bf Predicates] Logical constants {\bf true} and {\bf false},
%% uninterpreted propositional symbols, all Boolean connectives,
%% if-then-else, and binary predicates on two bit-vectors of the same
%% length, including equality and signed and unsigned comparisons.

%% \item[\bf Basic bit-vector operations]
%% Bit-Vector constants, and if-then-else.

%% \item[\bf Arithmetic]  signed and unsigned twos-complement
%% arithmetic operations on bit-vector, including unary minus, addition,
%% subtraction, multiplication, and signed and unsigned integer division
%% and modulo.  The operands of these operation must be of equal length.

%% \item[\bf Bitwise operations]  All bitwise Boolean operations on bit-vectors.

%% \item[\bf Concatenation and extraction]  Concatenation of bit-vectors,
%% extraction of bitfields, fixed and variable left- and right-shifting,
%% and sign-extension.

%% \item[\bf Arrays]  One-dimensional arrays with bit-vector indices and
%% values, write (which updates an array, producing a new array value),
%% and read (which accesses the value at a particular array index).  STP
%% arrays are not {\em extensional,} meaning that arrays cannot be
%% compared for equality, because extensionality is more difficult to
%% implement and has no been necessary for applications so far.
