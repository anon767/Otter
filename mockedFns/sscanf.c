...
static char *o_collect(register int c, register sym_file_stream_t *stream, char type, unsigned int width, int *basep)
{
...
int _doscan(register sym_file_stream_t *stream, const char *format, va_list ap)
{
...
int sscanf(const char *s, const char *format, ...)
{
        ...
        IOSIM_fd[scanf_counter] = malloc(sizeof(sym_file_stream_t));
        IOSIM_fd[scanf_counter]->offset = 0;
        IOSIM_fd[scanf_counter]->fd = scanf_counter;
        IOSIM_fd[scanf_counter]->sym_file = malloc(sizeof(sym_file_t));
        IOSIM_fd[scanf_counter]->sym_file->contents = s;
        IOSIM_fd[scanf_counter]->sym_file->stat.st_size = strlen(s);

        va_start(ap, format);

        retval = _doscan(IOSIM_fd[scanf_counter], format, ap);
